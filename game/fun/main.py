...
def main_game():
    ...
    while playing:
        ...
        if action in ['1', '0','','Absolutely!!!','fight','Fight','f','F','1: fight']:
            monster = choose_monster(pc, mon_list)
            
            playing = battle(pc, monster, xp_thresholds)

            if '(Boss)' in monster.name:
                playing = battle(pc, monster, xp_thresholds, 1)
            ...
            user = input()
            if user in ['N','n','Quit','quit','q','Q','Exit','exit','No','no','False','false','f','0','stop','2']:
                playing = False
